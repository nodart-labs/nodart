...
module.exports = async ({ app, cmd }) => {
  ...
  if ($.isEmpty(app.config.get.orm)) {
    console.error(
      `No data supplied for database configuration in directory "${cmd.appCmdDir}".`,
    );
    process.exit(1);
  }
  ...
  const fetchMigrations = (name, migrations = []) => {
    ...
    if (!name.match(pattern)) {
      console.error(
        `The source name "${name}" is not match pattern: A-z/_-0-9`,
      );
      process.exit(1);
    }

    migrations = object.uniq(
      migrations.map((m) => {
        m = m.toString().replace(/\//g, "").replace(/\\/g, "");

        if (!m.match(pattern)) {
          console.error(
            `The migration name "${m}" is not match pattern: A-z_-0-9`,
          );
          process.exit(1);
        }
  ...
  function getMigrationSources(exclude = []) {
    ...
    const sources = fs.dir(sourcesDir, ({ file }) => {
      ...
      return file.endsWith(".js")
        ? fs.skipExtension(
            $.trimPath(fs.formatPath(file.replace(sourcesDir, ""))),
          )
        : false;
    });
  ...
  return {
    async make(name) {
      ...
      await orm()
        ...
        .then(() =>
          console.log(
            `A new migration "${name}" was created in directory ${fs.path(
              orm().config.migrations.directory,
            )}`,
          ),
        );
    },
    ...
    makeSource(name, migrations = []) {
      ...
      const sourcePath = app.builder.envIsCommonJS
        ? "migrate/js/migrate_source"
      ...
      getSource(
        sourcePath,
        (file) => {
          const pathName = $.camel2Snake(
            $.hyphen2Camel(fs.formatPath(name)),
          ).toLowerCase();
          ...
          dest = fs.path(
            dest,
            $.camel2Snake(name).toLowerCase() +
              (app.builder.envIsCommonJS ? ".js" : ".ts"),
          );

          if (fs.isFile(dest)) {
            console.log(
              `A migration source by path ${dest} is already in use.`,
            );
            process.exit(1);
          }
            ...
            migrations.forEach((name) => {
              out.push(
                parts[1]
                  ...
                  .replace(/\[(TABLE)]/g, $.camel2Snake(name)),
              );
            });
            out.push(parts[2]);
            output = out.join("");
    ...
    async up() {
      ...
      await orm()
        .migrator()
        .up()
        .then(() => console.log("migrate up complete."));
      process.exit();
    },
    ...
    async down() {
      ...
      await orm()
        .migrator()
        .down()
        .then(() => console.log("migrate down complete."));
      process.exit();
    },
    ...
    async version() {
      ...
      await orm()
        .migrator()
        .currentVersion()
        .then((ver) => console.log(`the current migration version is: ${ver}`));
      process.exit();
    },
    ...
    async unlock() {
      ...
      await orm()
        ...
        .unlock()
        .then(() =>
          console.log("the migrations lock table forcibly unlocked."),
        );
      process.exit();
    },
  };
