...
module.exports = async ({ app, cmd }) => {
  ...
  async function seedSource(name, seeds = []) {
    ...
    if (source) {
      await seeder
        .run()
        .then(() => console.log(`seed source "${name}" is complete.`));
    } else {
  ...
  return {
    async make(name) {
      ...
      await orm()
        ...
        .then(() =>
          console.log(
            `A new seed "${name}" was created in directory ${fs.path(
              orm().config.seeds.directory,
            )}`,
          ),
        );
    },
