...
class CommandLine {
    ...
    execute(executor) {
        return __awaiter(this, void 0, void 0, function* () {
            const data = typeof executor === "function"
                ? yield executor({ app: this.app, cmd: this })
                : executor;
            if (!(typeof data === "object"))
                return;
            const { func, values } = (yield this.parser
...
class SystemCommandLine {
    ...
    init() {
        ...
        utils_1.fs.isFile(dest) ||
            utils_1.fs.copy((0, app_config_1.getSourcesDir)(app_config_1.DEFAULT_CMD_DIR + "/index.js"), dest);
    ...
    fetchAppState(app) {
        this.cmd.lock();
        utils_1.fs.include(this.cmd.appCmdDir, {
...
class CommandLineParser {
    static parseCommand() {
        ...
        args.forEach((arg, i) => {
            ...
            if (i === 0) {
                ...
                cmd.command = arg;
                args[i + 1] &&
                    (((_a = args[i + 1]) === null || _a === void 0 ? void 0 : _a.startsWith(exports.OPTION_POINTER)) ||
                        (cmd.action = args[i + 1]));
                return;
            }
            if (arg.startsWith(exports.OPTION_POINTER)) {
                ...
                cmd.options[arg] =
                    (_b = CommandLineParser.assignCommandOptionValues(args, arg, i)) !== null && _b !== void 0 ? _b : true;
            }
        });
        return cmd;
    }
    ...
    parseAction(command, executorData) {
        ...
        const actionFunc = action && typeof (executorData === null || executorData === void 0 ? void 0 : executorData[action]) === "function"
            ? executorData[action]
            : null;
        if (!actionFunc)
            return;
    ...
    validate(command, executorData) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!actionData.action && !command.action) {
                actions.length &&
                    errors.push(`No action supplied for command "${command.command} ${command.action}". (${actions.join("|")})`);
            ...
            if (typeof executorData[actionData.action] === "function") {
                ...
                missing.length &&
                    errors.push(`Missing required arguments: ${missing
                        .map((v) => exports.OPTION_POINTER + v)
                        .join(", ")}.`);
            }
            else
