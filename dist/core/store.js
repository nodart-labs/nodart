...
class State {
    ...
    getStore(storeName) {
        ...
        const store = storeObject && typeof storeObject === "object"
            ? utils_1.object.copy(storeObject)
            : {};
    ...
    checkState(store, state, debug = true) {
        ...
        return !Object.keys(state).some((prop) => {
            if (store.states.hasOwnProperty(prop) === false) {
                debug &&
                    console.error(`The state property "${prop}" is not defined in store.`);
                return true;
            }
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const execute = (listener, entries, type) => __awaiter(this, void 0, void 0, function* () {
                for (const [name, args] of entries) {
                    listener[type] === name &&
                        (listener.result = yield listener.action.apply(listener, args));
                }
            });
    ...
    remove(storeName, listeners) {
        ...
        event &&
            Object.keys(event).forEach((e) => (eventListeners[e] = event[e].toString()));
        state &&
            Object.keys(state).forEach((s) => (stateListeners[s] = state[s].toString()));
