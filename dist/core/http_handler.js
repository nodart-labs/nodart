...
class HttpHandler {
    ...
    setCors() {
        if (this.app.config.get.http.useCors) {
            ...
            if (this.request.method === 'OPTIONS') {
                this.response.writeHead(http_1.HTTP_STATUS.OK, { 'Content-Type': http_client_1.JSON_CONTENT_TYPE });
                this.response.end();
                return;
            }
        }
    ...
    fetchRequestData(callback, onError) {
        ...
        this.url = url;
        this.http = (0, app_1.loaders)().http.call([this.app, {
                host: this.app.host,
                uri: this.app.uri,
                query,
                queryString,
    ...
    runController(controller, route) {
        ...
        if (typeof controller[action] !== 'function' || (http_1.HTTP_METHODS.includes(action) && action !== this.http.method))
