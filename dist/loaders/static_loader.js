...
class StaticLoader extends app_loader_1.AppLoader {
    constructor(app) {
        ...
        this.repository =
            app.config.get.static.dirname || app_config_1.DEFAULT_STATIC_REPOSITORY;
    }
    call(args, path) {
        var _a, _b;
        var _c, _d;
        ...
        if (path === "/") {
            ...
            return (StaticLoader.cache[config.rootDir][path] = utils_1.fs.isFile(indexPath)
                ? indexPath
                : "");
        }
    ...
    serve(path, scope) {
        ...
        readStream.on("error", (err) => __awaiter(this, void 0, void 0, function* () {
            ...
            StaticLoader.cache[scope.app.rootDir][scope.request.url] = path.endsWith("favicon.ico")
                ? false
                : "";
        }));
        readStream.on("close", () => {
            file.error ||
                http_client_1.HttpClient.getResponseIsSent(scope.response) ||
                scope.response.writeHead(http_1.HTTP_STATUS.OK, {
