...
export class SessionLoader extends AppLoader {
  call(
    args: [http: HttpContainerInterface, config?: SessionClientConfigInterface],
  ): Session {
  ...
  ): SessionClientConfigInterface {
    return config
      ? ({
          ...(this.app.config.get.http?.session?.config || {}),
          ...config,
        } as SessionClientConfigInterface)
      : this.app.config.get.http?.session?.config ||
          ({} as SessionClientConfigInterface);
  }
