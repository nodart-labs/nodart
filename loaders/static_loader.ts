...
export class StaticLoader extends AppLoader {
  ...
  constructor(readonly app: App) {
    ...
    this.repository =
      app.config.get.static.dirname || DEFAULT_STATIC_REPOSITORY;
  }
  ...
  call(args: [config: AppConfigInterface], path: string): string {
    ...
    if (path === "/") {
      ...
      return (StaticLoader.cache[config.rootDir][path] = fs.isFile(indexPath)
        ? indexPath
        : "");
    }
  ...
  serve(
    ...
    scope.mimeTypes ||= HttpClient.mimeTypes(
      scope.app.config.get.http?.mimeTypes,
    );
    ...
    readStream.on("error", async (err) => {
      ...
      if (file.exists)
        return scope.app.resolveException(
          new HttpException({
            exceptionMessage: `Could not read data from file "${path.replace(
              this.getRepo(scope.app.rootDir),
              "",
            )}".`,
            exceptionData: err,
          }),
      ...
      StaticLoader.cache[scope.app.rootDir][scope.request.url] = path.endsWith(
        "favicon.ico",
      )
        ? false
        : "";
    ...
    readStream.on("close", () => {
      file.error ||
        HttpClient.getResponseIsSent(scope.response) ||
        scope.response.writeHead(HTTP_STATUS.OK, {
          "Content-Type": HttpClient.getDefaultContentType(
            extension,
            scope.mimeTypes,
            FILE_CONTENT_TYPE,
          ),
      ...
      scope.callback(
        file.exists || StaticLoader.cache[scope.app.rootDir][scope.request.url],
      );
    });
