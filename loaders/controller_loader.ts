...
export class ControllerLoader extends AppLoader {
  ...
  getDependency(
    ...
    property: string,
    dependency: typeof Model | typeof Service,
  ): any {
    switch (property) {
      case "service":
  ...
  getControllerByRouteDescriptor(
    app: App,
    route: RouteData,
    http: HttpContainer,
  ): BaseController | void {
    ...
    if (controller) {
      if (false === object.isProtoConstructor(controller, BaseController))
        throw `Controller loader: The provided type "${
          object.getProtoConstructor(controller)?.name
        }" is not a "Controller".`;
