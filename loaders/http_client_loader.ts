...
export class HttpClientLoader extends AppLoader {
  call(
    args: [
      app: App,
      config: Omit<
        HttpContainerConfigInterface & HttpDataInterface,
        "method" | "data" | "form"
      >,
    ],
  ): HttpContainer {
    ...
    const config = {
      ...app.config.get.http,
      ...(args[1] || {}),
    } as HttpContainerConfigInterface & HttpDataInterface;
    ...
    container.assignData({
      onSetResponseData:
        config.onSetResponseData ||
        function (data: HttpResponseData) {
          SYSTEM_STORE.events.HTTP_RESPONSE(
            app,
            container.getHttpResponse(data),
          );
        },
      onError:
        config.onError ||
        function () {
          app.resolveException(
