...
export = <AppEventHttpRequestInterface>((
  app: App,
  ...
  handler.serveStatic((result) => {
    ...
    if (result === false) {
      response.writeHead(HTTP_STATUS.NO_CONTENT, {
        "Content-Type": HTTP_CONTENT_MIME_TYPES.icon,
      });
      response.end();
    ...
    handler.fetchRequestData(() => {
      const route = app.router.getRouteByURLPathname(
        handler.url,
        handler.http.method,
      );
      ...
      new Promise((resolve) => {
        if (
          false === handler.runHttpService(route) &&
          false === handler.initController(route)
        )
          throw new HttpException(handler.http, {
