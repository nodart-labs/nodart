...
export class HttpHandler {
    ...
    public static warnings: { useCors: boolean, serveStatic: boolean, fetchDataOnRequest: boolean } = {
        useCors: false,
        serveStatic: false,
    ...
    setCors() {
        ...
        if (this.app.config.get.http.useCors) {
            ...
            if (this.request.method === 'OPTIONS') {
                this.response.writeHead(HTTP_STATUS.OK, {'Content-Type': JSON_CONTENT_TYPE})
                this.response.end()
                return
            }
    ...
    serveStatic(callback: (result?: string | boolean | undefined) => any) {
        ...
        if (this.app.config.get.static.serve) {
            ...
            const pathname = this.request.url.includes('?') ? this.request.url.split('?')[0] : this.request.url
    ...
    fetchRequestData(callback: (data?: JSONLikeInterface) => any, onError?: (err: Error) => any) {

        const {url, query, queryString} = HttpClient.parseURL(this.request.url)

        this.url = url

        this.http = loaders().http.call([this.app, {
            host: this.app.host,
            uri: this.app.uri,
            query,
            queryString,
        ...
        if (this.app.config.get.http.fetchDataOnRequest) {
            ...
            ['POST', 'PUT', 'PATCH'].includes(this.request.method) ? this.http.fetchData(callback, onError) : callback()
    ...
    initController(route: RouteData): false | void {
        ...
        const controller = loaders().controller.getControllerByRouteDescriptor(this.app, route, this.http)

            || loaders().controller.getControllerByRoutePath(this.app, route, this.http)

        if (!controller) return false

        this.processData(controller[CONTROLLER_INITIAL_ACTION](), () => this.runController(controller, route))
    }
    ...
    runController(controller: BaseController, route: RouteData) {
        ...
        if (typeof controller[action] !== 'function' || (HTTP_METHODS.includes(action) && action !== this.http.method))
    ...
    processData(data: any, callback?: Function) {
