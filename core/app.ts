...
export const loaders = () =>
  App.system.state.loaders as AppStateLoadersInterface;
...
export class App {
  ...
  protected _isServe = false;

  private _host: HttpHost = {
    port: null,
    protocol: null,
    host: null,
    hostname: null,
  ...
  async start(
    ...
    serve?: (
      app: App,
    ) => Server | SecureServer | Promise<Server | SecureServer>,
  ): Promise<{
    app: App;
  ...
  private setHost(
    connection: { address: string; family: string; port: number },
    protocol: HttpProtocols,
  ) {
    this._host = Object.freeze(
      HttpClient.fetchHostData({
        port: connection.port,
        protocol,
        host: connection.address,
  ...
  resolveHttpRequest(req: Http2ServerRequest, res: Http2ServerResponse) {
    this.service.requestPayload
      ? this.service
          ...
          .then(() => SYSTEM_STORE.events.HTTP_REQUEST(this, req, res))
  ...
  static get system() {
    ...
    return {
      ...
      set: async (data: StoreListenerArguments) =>
        await store.set(SYSTEM_STATE_NAME, data),
      listen: async (data: StoreListenerArguments) =>
        await store.listen(SYSTEM_STATE_NAME, data),
...
export class AppFactory {
  ...
  createState() {
    App.system.store ||
      Store.add(
        SYSTEM_STORE_NAME,
        fs.path(__dirname, "../" + SYSTEM_STORE_REPOSITORY),
      );
...
export abstract class AppModule {
  protected constructor(
    readonly app: App,
    readonly config: AppModuleConfigInterface = { options: {} },
  ) {}

  abstract init<K extends keyof AppEmitterEvents>(
    event: K,
    scope: AppEmitterEvents[K],
  ): void | AppModuleFacade;
}
...
export class AppServiceManager {
  ...
  protected _exceptionPayload: (
    data: HttpResponseDataInterface,
  ...
  get module() {
    return (this._module ||= new ModuleService(
      this.app,
...
export class AppEnv {
  private envFilenamePattern = /^[A-z\d.-_]+(\.ts|\.js)$/;

  private tsConfigFileName = "tsconfig.json";
  ...
  get baseDir() {
    return fs.isFile(fs.path(this.app.rootDir, this.tsConfigFileName))
      ? this.app.rootDir
      : process.cwd();
  }
...
export class AppEmitter {
  ...
  async emit<K extends keyof AppEmitterEvents>(
    event: K,
    scope?: AppEmitterEvents[K],
  ) {
...
export class AppExceptionResolve {
  ...
  getHandler(): ExceptionHandler {
    return (this._handler ||= this.app
      .get("exception_handler")
      .call([this.exception]) as ExceptionHandler);
  }

  getLog(): ExceptionLog {
    return (this._log ||= this.app
      .get("exception_log")
      .call([this.exception]) as ExceptionLog);
  }
