  ...
  static add(store: string, storeDir: string): void {
    if (!fs.isDir(storeDir))
      throw new RuntimeException(
        `Store: The store repository "${storeDir}" does not exist.`,
      );

    if (Store.stores[store])
      console.info(
        `Store: The store name "${store}" already has been registered in store.`,
      );
...
export class State {
  ...
  protected getStore(storeName: string): StoreStateObject | void {
    ...
    const store =
      storeObject && typeof storeObject === "object"
        ? object.copy(storeObject)
        : {};
  ...
  checkState(
    ...
    return !Object.keys(state).some((prop) => {
      if (store.states.hasOwnProperty(prop) === false) {
        debug &&
          console.error(
            `The state property "${prop}" is not defined in store.`,
          );
  ...
  on(storeName: string, listeners: StoreListeners): void {
    ...
    const setAction = (src, type) => {
      src &&
        Object.entries(src).forEach(([name, action]) => {
          if (!(name in store[type])) {
            return console.error(
              `The ${types[type]}'s property "${name}" is not defined in "${storeName}" store.`,
            );
          }
  ...
  async listen(storeName: string, stateData: StoreListenerArguments) {
    ...
    const execute = async (listener, entries, type) => {
      for (const [name, args] of entries) {
        listener[type] === name &&
          (listener.result = await listener.action.apply(listener, args));
      }
    };
  ...
  remove(storeName: string, listeners: StoreListeners): void {
    ...
    event &&
      Object.keys(event).forEach(
        (e) => (eventListeners[e] = event[e].toString()),
      );

    state &&
      Object.keys(state).forEach(
        (s) => (stateListeners[s] = state[s].toString()),
      );
    ...
    const match = (listener) => {
      ...
      return listener.event
        ? events.some(
            (e) => listener.event === e && action === eventListeners[e],
          )
        : states.some(
            (s) => listener.state === s && action === stateListeners[s],
          );
    };

    store.listeners.forEach(
      (listener, index) => match(listener) && store.listeners.splice(index, 1),
    );
  }
}
