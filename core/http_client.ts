...
export class HttpContainer implements HttpContainerInterface {
  ...
  constructor(
    readonly config: HttpContainerConfigInterface & HttpDataInterface,
  ) {}
  ...
  get method(): HttpMethod {
    return (this.config.method ||=
      this.request.method.toLowerCase() as HttpMethod);
  }
  ...
  get form(): HttpFormData {
    return (this._form ||= new HttpFormData(
      this,
      this.config.form,
    )) as HttpFormData;
  }
  ...
  get respond(): HttpResponder {
    ...
    const engineConfig =
      this.config.engine?.config || ({} as EngineClientConfigInterface);
    const engine =
      typeof this.config.engine?.client === "function"
  ...
  get session(): Session {
    ...
    const sessionConfig =
      this.config.session?.config || ({} as SessionClientConfigInterface);

    return (this._session =
      typeof this.config.session?.client === "function"
  ...
  getHttpResponse(
    assignResponseData?: HttpResponseData,
  ): BaseHttpResponseInterface {
  ...
  send(
    content: JSONObjectInterface | string,
    ...
    this.setResponseData({
      ...
      contentType:
        contentType ||
        HttpClient.getDefaultContentType("json", this.config.mimeTypes),
  ...
  sendText(
    ...
    this.setResponseData({
      ...
      contentType:
        contentType ||
        HttpClient.getDefaultContentType(
          "text",
...
export class HttpFormData implements HttpFormDataInterface {
  ...
  protected _stat: {
    fields: {
      [field: string]:
        | HttpFormDataClientsField
        | Array<HttpFormDataClientsField>;
    };
  ...
  constructor(
    ...
    this.config.uploadDir = fs.isDir(this.config.uploadDir)
  ...
  get form() {
    return this.client({
      ...(this.config.options || {}),
      headers: this.http.request.headers,
    });
  }
  ...
  protected _onFieldUpload(
    ...
    if (field in this.fields) {
      Array.isArray(this.fields[field]) ||
        (this.fields[field] = [this.fields[field]]);

      Array.isArray(this._stat[field]) ||
        (this._stat[field] = [this._stat[field]]);
...
export class HttpClient {
  ...
  static mimeTypes(assign?: HttpMimeTypes) {
    return assign
      ? { ...HTTP_CONTENT_MIME_TYPES, ...assign }
      : HTTP_CONTENT_MIME_TYPES;
  }

  static getHttpResponseData(
    http: BaseHttpResponseInterface,
    assignData?: HttpResponseData,
  ): HttpResponseData {
    http.responseData ||= {};
    ...
    http.responseData.contentType =
      assignData?.contentType ||
      http.responseData.contentType ||
      http.response.getHeader("content-type");
  ...
  static getDefaultContentType(
    ...
    return (
      mimeTypes[entry] || HTTP_CONTENT_MIME_TYPES[entry] || defaultMimeType
    );
  }
  ...
  static getDataFromStatusCode(
    ...
    const status =
      http.responseData.status ?? statusOnNone ?? http.response.statusCode;
    const contentType =
      http.responseData.contentType ??
      http.response.getHeader("content-type") ??
      JSON_CONTENT_TYPE;
  ...
  static fetchHostData(httpHost: HttpHost): HttpHost {
    const { port, protocol, host, hostname } = require("url").parse(
      HttpClient.getURI(httpHost),
    );
  ...
  static getURI(data: HttpHost) {
    return (
      `${data.protocol.replace(":", "")}://${data.host.replace(
        ":" + data.port,
        "",
      )}` + (data.port ? ":" + data.port : "")
    );
  ...
  static throwBadRequest(message = "") {
    throw new HttpException(
      message ||
        "The current HTTP method receives no response from the request method.",
      {
  ...
  static throwNoContent(message = "") {
    throw new HttpException(
      message ||
        "The current HTTP request receives no content from the response.",
      {
